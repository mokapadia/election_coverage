...
class NewsApi:
    ...
    def time_count(self):
        ...
        res = list(
            self.collection.aggregate(
                [{"$project": {"publishedAt": {"$substr": ["$publishedAt", 0, 10]}}}]
            )
        )
    ...
    def character_count(self):
        ...
        res = list(
            self.collection.find(
                {"character_count": {"$exists": "true", "$lt": 40000}},
    ...
    def topic_analyzer(self):
        ...
        plt.pie(
            mongo_df["count"], labels=mongo_df["_id"], autopct="%1.1f%%", startangle=140
        )
    ...
    def sentiment(self, source_lst, avg=None, keyword=None):
        ...
        if avg:
            pipeline = [
                {"$match": {"source.name": {"$in": source_lst}}},
                {"$group": {"_id": "$source.name", "avg": {"$avg": "$sentiment"}}},
            ]
            ...
            res = list(
                self.collection.find(
                    {"source.name": {"$in": source_lst}},
                    {"_id": 0, "source.name": 1, "sentiment": 1},
                )
            )
            sentiment = {}
            ...
            if keyword:
                res = list(
                    self.collection.find(
                        {"source.name": {"$in": source_lst}, "category": keyword},
                        {"_id": 0, "source.name": 1, "sentiment": 1, "category": 1},
                    )
                )
                sentiment = {}
                sentiment["source"] = [i["source"]["name"] for i in res]
    ...
    def centrist_hist(self, centrist, source=None):
        ...
        if source:
            ...
            plt.ylim((-0.4, 0.4))
            plt.axhline(
                y=centrist_df["score"].mean(axis=0),
                color="gray",
                linestyle="--",
                linewidth=2,
                label="Average Centrist Sentiment Score",
            )
            plt.legend()
    ...
    def left_vs_right(self, right, left, center, source=None):
        ...
        if source:
            group = pd.concat(
                [self.sentiment(right, avg=True), self.sentiment(left, avg=True)]
            )
    ...
    def bias_by_topic(self, right, left, keyword=None):
        ...
        if keyword:
            ...
            plt.title(
                f"Seniment Scores of Articles in {keyword} Category - Right vs. Left"
            )
    ...
    def compare_two_sources(self, source1, source2, category=None):
        if category:
            ...
            plt.title(
                f"Seniment Scores of Articles in {category} Category - {source1} vs. {source2}"
            )
        else:
            total_df = self.sentiment([source1, source2])
            plt.title(
                f"Seniment Scores of Articles in ALL Categories - {source1} vs. {source2}"
            )

        colors = [
            "red" if source == source1 else "blue" for source in total_df["source"]
        ]
    ...
    def anova(self, right, left, center):
        ...
        p_var = levene(centrist_scores["avg"], left_scores["avg"], right_scores["avg"])[
            1
        ]
        ...
        if (p1 > 0.05) & (p2 > 0.05) & (p3 > 0.05) & (p_var > 0.05):
            f_statistic, p_value = f_oneway(
                centrist_scores["avg"], left_scores["avg"], right_scores["avg"]
            )
            print("ANOVA results:")
            ...
            affiliation = (
                (["left-wing"] * len(left_scores))
                + (["centrist"] * len(centrist_scores))
                + (["right-wing"] * len(right_scores))
            )
